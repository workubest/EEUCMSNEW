                      ...
                      item.subItems ? (
                        <Collapsible key={item.title} defaultOpen={item.subItems.some(sub => isActive(sub.url))}>
                          <CollapsibleTrigger asChild>
                            <SidebarMenuButton className="h-12 px-3 rounded-lg transition-all duration-200 hover:bg-eeu-orange/10 hover:shadow-md w-full justify-start">
                              <item.icon className="h-5 w-5 text-eeu-orange" />
                              {!isCollapsed && <span className="text-base font-medium text-gray-700">{item.title}</span>}
                              {!isCollapsed && <ChevronDown className="h-5 w-5 ml-auto transition-transform group-data-[state=open]/collapsible:rotate-180" />}
                            </SidebarMenuButton>
                          </CollapsibleTrigger>
                                ...
                                <SidebarMenuItem key={subItem.title}>
                                  <SidebarMenuButton asChild className="h-12 px-3 rounded-lg transition-all duration-200 hover:bg-eeu-orange/10 hover:shadow-md">
                                    <NavLink
                                      ...
                                      className={({ isActive }) => `${getNavClass(isActive)} ${isActive ? 'bg-gradient-to-r from-eeu-orange to-eeu-green text-white shadow-lg' : 'text-gray-700'}`}
                                    >
                                      <subItem.icon className="h-5 w-5 text-eeu-orange" />
                                      {!isCollapsed && <span className="text-base font-medium text-gray-700">{subItem.title}</span>}
                                    </NavLink>
                                  </SidebarMenuButton>
                                </SidebarMenuItem>
                              ))}
