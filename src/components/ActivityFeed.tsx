...
export default function ActivityFeed() {
  ...
  const fetchActivities = async () => {
    try {
      // Use GAS proxy to fetch recent activities
      ...
      if (data.success && data.data) {
        const activities: Activity[] = data.data.map((item: any) => ({
          id: item.id || item.ID,
          action: item.action || item.Action || 'unknown',
          userName: item.userName || item['User Name'] || 'Unknown',
          createdAt: item.createdAt || item['Created At'],
          ticketNumber: item.ticketNumber || item['Ticket Number']
        }));
